...
11. 撤销更改 可以通过使用`git clean` `git revert` `git reset` `git rm`
    ...
    * 通过使用`git revert`就是可以将已经提交到历史记录的文件进行撤销的操作，通过使用这种方式会撤销单个提交，不会通过删除所有的后续的提交来回复到项目的先前的状态，这种方式不会更改项目的历史记录，这种操作是一个安全的操作。会创建一个新的提交，该提交会撤销指定的更改。
    * 通过使用`git reset`主要有三种调用的方式，这些形式分别对应着`--soft\--mixed\--hard`这三种参数分别对应着git的三个内部的状态管理机制，提交树(HEAD)，暂存索引，工作目录。
    > git中的三个树 一个就是工作目录，该树与本地文件系统同步，代表着文件和目录中的内容所做的即时更改，另外一个是暂存索引表示的正在跟踪工作目录更改，这些更改通过使用`git add`来进行提交到暂存区中，最后一个就是提交历史记录，通过使用`git commit`命令将更改添加到历史记录中的永久的快照中。该快中澳还包括提交时暂存索引的状态。
    * 通过使用`git reset`的作用效果是这样的![alt text](image-11.png)
    * 通过使用`git checkout`的作用是这样的![alt text](image-10.png)
    * `git rm`用于在git存储库中删除文件，可以认为他与`git add`命令正好相反
12. 重写历史记录
    * 通过使用`git commit --amend`来对最近的提交的修改
    * 通过使用这种方式可以