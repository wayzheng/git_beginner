...
12. 重写历史记录
    * 通过使用`git commit --amend`来对最近的提交的修改 可以将暂存的变更与先前的变更进行合并，然后创建一个全新的提交，他可以在不更改快照的情况下简单编辑先前的提交的信息，但是amend不只是改变最近的提交，他将完全取代最近的提交
    * `git rebase`就是一种变基操作，专门用于将变更从一个分支集成到另外一个分支，另外一个集成使用程序是`git merge`合并始终是向前移动的变更记录，变基具有强大的历史记录重写功能
    * 变基是将一系列提交移动或者合并到新的基本提交的流程，在功能分支工作流程的上下文中，变基最为有用并且易于可视化，从内容上来看，变基就是将分支的基础从一个提交变为另外一个提交，使得它看起来好像是从另一个提交中创建分支一样，在内部，git通过创建新的提交并将其应用到指定的基础上来实现此目的，尽管分支看起来一样，但是他是由全新的提交组成的
13. 通过使用`git remote`命令实现远程仓库与本地仓库连接
    * 通过使用github或者gitee就是可以创建新的代码仓库，通过使用`git remote`可以列出当前的代码仓库与其他远程仓库连接，通过使用`git remote -v`可以在上面功能的基础之上在显示每一个连接的url
    * 通过使用`git remote add <name> <url>`就是可以将当前仓库与url对应的远程代码仓库连接在一起，并且会设置这个代码仓库的名称是`<name>`    
